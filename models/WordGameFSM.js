...
class WordGameFSM {
  constructor(gameId) {
    ...
    this.guessedWords = [];
    this.gameResponses = []; // New list for storing game responses
    this.responseCounter = 1; // Counter for response IDs
  }
  ...
  addResponse(response) {
    response.id = this.responseCounter++;
    this.gameResponses.push(response);
    console.log("this.gameResponses", this.gameResponses);
  }
  ...
  getCurrentState() {
    return {
      ...
      words: this.words,
      gameResponses: this.gameResponses, // Include responses in the game state
    };
  }
}
